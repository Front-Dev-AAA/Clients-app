            ...
            deleteClient.deleteModalDelete.addEventListener('click', () => {
                try {
                    ...
                    setTimeout(() => {
                        ...
                        let status = deleteClientItem(data.id);

                        // для наглядности создаем перемнную PromiseResult
                        let PromiseResult = status.then(function (value) {
                            if ((value === 405) || (value === 404)) {
                                console.log('сервер сломан');
                                 deleteClient.deleteModalText.textContent = 'Опять ты?! Сервер сломан! Иди учи уроки!!!'; 
                            }
                            console.log(`Из промиса получены данные: ${value}`);
                            if (value < 400) {
                                // удаление из таблицы данных, что видит клиент 
                               document.getElementById(data.id).remove();
                            //    закрытие модалки автоматом
